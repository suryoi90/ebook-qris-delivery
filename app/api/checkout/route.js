  ...
  try {
    ...
    if (data.token) {
      return NextResponse.json({
        ...
        token: data.token,
        redirec
# 1. Reset riwayat commit lokal ke posisi GitHub yang bersih
git reset origin/main

# 2. Hapus semua file script sementara yang menampung teks kunci
Remove-Item -Force fix-*.js, update-*.js, deploy-*.js, hardcode-*.js, sync-*.js, test-*.js -ErrorAction SilentlyContinue

# 3. Tulis file Backend yang bersih dan aman
$checkoutCode = @'
import { NextResponse } from 'next/server';
